...
const tableData = {
  ...
  widthArr: [70, 120, 100, 100, 120, 120, 100, 100, 100],
  tableData: [],
};

const PayableInvoices = (props) => {
  const {
    isVisible,
    payableInvoices,
    setPayableInvoices,
  ...
  useEffect(() => {
    if (isVisible && payableInvoices?.length > 0) {
      setData({
        ...data,
        tableData: payableInvoices.map(
          (
            {
              dateOfTransaction,
              documentNumber,
  ...
  const exportDataToExcel = async (exData) => {
    const data = (exData === false ? [{}] : exData).map((item, index) => {
      const arr = [
        { No: index + 1 },
        { Tarix: item.dateOfTransaction },
        { Sənəd: item.documentNumber },
        { Qaimə: item.invoiceNumber },
        {
          Məbləğ: `${formatNumberToLocale(defaultNumberFormat(item.amount))}  ${
            item.currencyCode
          }`,
        },
        {
        ...
        {
          "Ödəniş növü":
            item.cashboxTypeId === 1
              ? "Nağd"
              : item.cashboxTypeId === 2
              ? "Bank"
              : item.cashboxTypeId === 3
              ? "Kart"
              : "Digər",
        },
